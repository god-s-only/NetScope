...
data class TrafficFilter(
    ...
    fun matches(transaction: HttpTransaction): Boolean {
        if (searchQuery.isNotBlank()) {
            ...
            val matchesBody = transaction.responseBody
                ?.lowercase()?.contains(q) == true
            val matchesPath = transaction.path.lowercase().contains(q)
            if (!matchesUrl && !matchesHost && !matchesBody && !matchesPath) {
                return false
            }
        }
        if (methods.isNotEmpty() && transaction.method !in methods) return false
        if (statusCategories.isNotEmpty() &&
            transaction.statusCategory !in statusCategories) return false
        if (showSlowOnly && !transaction.isSlow) return false
        if (showErrorsOnly && !transaction.isClientError &&
            !transaction.isServerError) return false
        if (appPackages.isNotEmpty() &&
            transaction.appInfo?.packageName !in appPackages) return false
